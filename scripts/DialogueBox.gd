...
func clear_dialogue_box():
	get_node("HBoxContainer/VBoxContainer/Text").text = ""
	for choice in choice_buttons:
		$HBoxContainer/VBoxContainer/VBoxContainer.remove_child(choice)
	choice_buttons = []
	
func add_text(text: String):
	get_node("HBoxContainer/VBoxContainer/Text").text = text
	pass
...
func add_choice(choice_text: String):
	...
	$HBoxContainer/VBoxContainer/VBoxContainer.add_child(button_obj)
	#print(button_label.size)
	#await get_tree().process_frame
	$HBoxContainer/VBoxContainer/VBoxContainer.get_child(button_obj.choice_index).custom_minimum_size.y = $HBoxContainer/VBoxContainer/VBoxContainer.get_child(button_obj.choice_index).get_child(0).size.y
